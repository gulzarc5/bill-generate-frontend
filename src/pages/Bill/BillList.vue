<template>
  <div class="q-pa-md">
    <q-btn color="primary" to="/bill/generate" label="Add New Bill" />
  </div>
  <div class="q-pa-md">
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td key="total_amount" :props="props">{{
            (parseFloat(props.row.total_amount) + parseFloat(props.row.round_off_amount)).toFixed(2) ?? 0.00 }}</q-td>
...
const fetchDataList = async () => {
  try {
    ...
    const response = await api.get('/api/admin/bill/list', { headers }); // Replace with your API endpoint
