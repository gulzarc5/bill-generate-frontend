<template>
  <div class="q-pa-md">
    <q-btn color="primary" @click="showModalAdd=true" label="Add New Brand" />
  </div>
  <div class="q-pa-md">
    <q-table
      flat bordered
  ...
  <q-dialog v-model="showModal" persistent>
    <q-card flat bordered :class="{ 'mobile-style-card': $q.platform.is.mobile, 'my-card': !$q.platform.is.mobile }">
      <q-card-section style="text-align:center">
        <div class="text-h6">Edit Brand</div>
      </q-card-section>
      <q-card-section>
        <q-input v-model="brandID" class="hidden-input"
      hide-bottom-space label="Brand ID" required/>
        <q-input v-model="brandName" lazy-rules
                    :rules="[ val => val && val.length > 0 || 'Please Enter Brand Name']"
                    required label="Brand Name" />
      </q-card-section>
      <q-card-actions align="right">
        <q-btn color="green" @click="updateBrand" label="Update" />
  ...
  <q-dialog v-model="showModalAdd" persistent>
    <q-card flat bordered :class="{ 'mobile-style-card': $q.platform.is.mobile, 'my-card': !$q.platform.is.mobile }">
      <q-card-section style="text-align:center">
        <div class="text-h6">Add Brand</div>
      </q-card-section>

      <form @submit.prevent="createBrand" ref="myForm">
        ...
        <q-card-actions align="right">
          ...
          <q-btn color="red" type="button" @click="showModalAdd=false" label="Cancel" />
        </q-card-actions>
      </form>
    </q-card>
  </q-dialog>
...
const fetchDataList = async () => {
  try {
    ...
    const response = await api.get('/api/admin/brand/list', { headers }); // Replace with your API endpoint
...
function openModal(data) {
  ...
  brandID.value = category.id;
  brandName.value = category.name;
...
async function updateStatus(id) {
  ...
  await api
    .post('/api/admin/brand/status', data , {
      headers: {
            Authorization: `Bearer ${token}`
      }
    })
